...
func (api *Api) catchAll(w http.ResponseWriter, req *http.Request) {
	http.NotFound(w, req)
}
...
func (api *Api) allGigs(w http.ResponseWriter, _ *http.Request) {
	since := time.Date(1982, 10, 16, 18, 0, 0, 0, time.Local)
	api.gigsFrom(since, w)
}

func (api *Api) futureGigs(w http.ResponseWriter, _ *http.Request) {
	since := time.Now().Truncate(24 * time.Hour)
	api.gigsFrom(since, w)
}

func (api *Api) allMembers(w http.ResponseWriter, _ *http.Request) {
	members := api.members.FindAll()
	renderJson(w, members)
}

func (api *Api) actualMembers(w http.ResponseWriter, _ *http.Request) {
	members := api.members.FindActual()
	renderJson(w, members)
}

func (api *Api) allAlbums(w http.ResponseWriter, _ *http.Request) {
	albums := api.albums.Albums()
	renderJson(w, albums)
}

func (api *Api) onlyAlbums(w http.ResponseWriter, _ *http.Request) {
	result := api.albums.AlbumsOf(services.AlbumType)
	renderJson(w, result)
}

func (api *Api) onlySingles(w http.ResponseWriter, _ *http.Request) {
	result := api.albums.AlbumsOf(services.SingleType)
	renderJson(w, result)
}

func (api *Api) promo(w http.ResponseWriter, _ *http.Request) {
